...
public class RobotControllerTest {
	...
	public void should_set_initial_position() throws Exception {
		...
		// Then
		this.mockMvc.perform(post(RobotController.ROOT_URL).contentType(MediaType.TEXT_PLAIN_VALUE).content(script))
				.andExpect(status().isOk()).andExpect(jsonPath("$").isNotEmpty()).andExpect(jsonPath("$.row").value(1))
				.andExpect(jsonPath("$.col").value(3)).andExpect(jsonPath("$.direction").value("EAST"))
	...
	public void should_set_forward() throws Exception {
		...
		this.mockMvc.perform(post(RobotController.ROOT_URL).contentType(MediaType.TEXT_PLAIN_VALUE).content(script))
				.andExpect(status().isOk()).andExpect(jsonPath("$").isNotEmpty()).andExpect(jsonPath("$.row").value(1))
				.andExpect(jsonPath("$.col").value(4)).andExpect(jsonPath("$.direction").value("EAST"))
				.andExpect(jsonPath("$.directionDegree").value(90));
	}
	...
	public void should_set_turnaround() throws Exception {
		...
		this.mockMvc.perform(post(RobotController.ROOT_URL).contentType(MediaType.TEXT_PLAIN_VALUE).content(script))
				.andExpect(status().isOk()).andExpect(jsonPath("$").isNotEmpty()).andExpect(jsonPath("$.row").value(1))
				.andExpect(jsonPath("$.col").value(4)).andExpect(jsonPath("$.direction").value("WEST"))
	...
	public void should_set_right() throws Exception {
		...
		this.mockMvc.perform(post(RobotController.ROOT_URL).contentType(MediaType.TEXT_PLAIN_VALUE).content(script))
				.andExpect(status().isOk()).andExpect(jsonPath("$").isNotEmpty()).andExpect(jsonPath("$.row").value(1))
				.andExpect(jsonPath("$.col").value(4)).andExpect(jsonPath("$.direction").value("NORTH"))
